...
if (db && !error) {
  ...
  app.use(
    helmet({
      contentSecurityPolicy: { directives: buildCsp(isProduction) },
  ...
  app.use(
    cors({
      origin: (origin, callback) => {
        const allowedOrigins = [`http://localhost:${port}`, process.env.NODE_HOST].filter(Boolean);
...
Sitemap: ${siteUrl}/sitemap.xml`);
  ...
  app.use('*all', async (req, res) => {
    try {
      ...
      const rendered = await render(url, db, theme);
