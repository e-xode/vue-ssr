...
Both `src/plugins/vuetify.js` and this skill's `references/color-palette.md` are the only places hex values should ever appear. Components use semantic names (`color="primary"`), never raw hex.
...
- **Never hardcode hex** in a component or another skill — reference `references/color-palette.md` (or `src/plugins/vuetify.js`) by semantic token name.
