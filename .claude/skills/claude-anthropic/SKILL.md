...
15. **Always-loaded budget.** `CLAUDE.md` bytes + every skill description + every agent description load EVERY turn; the sum is capped at 43,000 chars (WARN) / 47,000 chars (ERROR), enforced by `scripts/audit.py` (which prints the current total as INFO on each run — treat it as a ratchet, never let it grow without cause). Sub-budget: skill descriptions alone stay ≤ 23,000 chars (audit-warned). The harness's own listing budget is **1 % of the model's context window**, scaled here by `skillListingBudgetFraction: 0.025` in `.claude/settings.json` — so the same config is comfortable on a 1M-context session and tight on a 200k one, which is what a forker will usually run. On overflow the listing keeps every skill *name* and drops *descriptions*, least-invoked first. Before trimming a description, reach for `disable-model-invocation: true` on skills that are only ever invoked deliberately. See [references/skill-anatomy.md](./references/skill-anatomy.md) and [references/case-studies.md](./references/case-studies.md) CS-7.
16. **An agent must be able to load the skills its body names.** A `tools:` line is an allowlist: omit `Skill` and the agent cannot invoke any skill, silently. Put non-negotiable skills in `skills:` (full content preloaded) and add `Skill` to `tools:` for discretionary ones. See [references/agent-anatomy.md](./references/agent-anatomy.md) — this is the most consequential frontmatter mistake available in this project.
