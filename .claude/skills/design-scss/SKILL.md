---
name: design-scss
description: "SCSS design-system reference for the Vue SSR Starter Kit (e-xode/vue-ssr): design tokens (spacing, border-radius, shadows, transitions, breakpoints in variables.scss), 11 mixins (flex-center, flex-between, flex-col, truncate, multiline-truncate, absolute-center, transition, hover-lift, button-reset, visually-hidden, respond-to), animation keyframes and utility classes (_animations.scss), utility classes (_utilities.scss: glass, badges, text gradients, hover effects, skeleton), auto-injection barrel (_inject.scss via Vite additionalData), and component-scoped SCSS file conventions. Trigger on ANY styling work: writing SCSS, choosing spacing or shadow tokens, responsive breakpoints, adding animations, creating component .scss files, Vuetify style overrides, layout utilities, CSS architecture decisions, or design-system questions. Don't use for: app architecture or file structure (→ vue-ssr-architecture), auth flow (→ vue-ssr-auth), Docker/CI (→ vue-ssr-deployment), post-task validation (→ vue-ssr-hooks), Vuetify component API usage (→ vuetify-components)."
---
