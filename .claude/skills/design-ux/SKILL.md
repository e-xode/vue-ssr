---
name: design-ux
description: "Design and UX methodology for the Vue SSR Starter Kit (e-xode/vue-ssr): design-first thinking, visual hierarchy (size > color > weight > position), Material Design 3 alignment via Vuetify 4, 8px spacing rhythm ($spacing-unit multiples), color semantics (primary for CTAs, success/error for feedback, neutral for chrome), micro-interaction philosophy (meaningful, restrained, accessible), responsive UX (mobile-first, breakpoint semantics), and WCAG 2.1 AA accessibility as a first-class requirement. Trigger on: UI quality concerns, design direction choices, aesthetic decisions, visual hierarchy review, spacing/layout critique, accessibility audit, responsive layout decisions, animation/transition design, color usage review, UX patterns, component composition, user feedback design. Don't use for: SCSS implementation details or mixin authoring (→ design-scss), Vuetify component API or prop configuration (→ vuetify-components), app architecture or routing (→ vue-ssr-architecture), auth flow (→ vue-ssr-auth)."
---
