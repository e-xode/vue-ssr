---
name: vue-ssr-auth
description: "Authentication and security reference for the Vue SSR Starter Kit: the email-based security code flow, session management, rate limiting/IP blocking, and reCAPTCHA v3 integration. Trigger on any auth work: login, signup, password reset, session management, security code, rate limiting, or captcha. Don't use for: general app architecture (→ vue-ssr-architecture), Docker/CI (→ vue-ssr-deployment), post-task validation (→ vue-ssr-validation), UI/UX design (→ design agent)."
---
