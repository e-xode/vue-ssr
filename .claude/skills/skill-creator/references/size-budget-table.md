...
| --------------------- | -------------------: | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `CLAUDE.md`           |          under 10 KB | hard failure in `claude-anthropic/scripts/audit.py` | This file loads every turn. It should contain hard rules and indexes, not full project knowledge.                               |
| `*/SKILL.md`          |      under 500 lines | hard failure in `claude-anthropic/scripts/audit.py` | A skill body should be fast to load and execute. Long examples, rubrics, and tables belong in references.                       |
| `*/references/*.md`   | warn above 300 lines | warning in `claude-anthropic/scripts/audit.py`      | References can be longer, but large files need a table of contents and clear section names so agents load only what they need.  |
