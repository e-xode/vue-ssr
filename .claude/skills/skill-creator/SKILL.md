...
- **Description budget.** The binding constraint here is not the per-skill listing cap but a **23,000-char
  ...
  anti-triggers only; enumerated knowledge belongs in the body.
- **Anti-triggers are mandatory.** Every description ends with `Don't use for: X (→ sibling-skill)`. `audit.py`
  warns when one is missing.
- **Size budgets.** Every file kind has a budget and an enforcement point — see
...
The skill root is `.claude/skills/skill-creator/`. The `python -m scripts.*` invocations below resolve from that
directory, so `cd` into it first:

```bash
cd .claude/skills/skill-creator
python3 -m scripts.aggregate_benchmark <workspace>/iteration-1 --skill-name <name>
python3 -m scripts.run_loop --eval-set <path> --skill-path <path> --model <session-model-id> --max-iterations 5
python3 eval-viewer/generate_review.py <workspace>/iteration-1 --skill-name <name>
```
...
Two edits above the rule, two single-line edits inside the body, one additive edit in a reference. Everything
else below the rule is byte-for-byte upstream, so a future re-sync is a straight replacement of that block.

1. `description` frontmatter rewritten for this project's trigger surface and anti-triggers.
2. This `## Project layer` section added.
3. Body, "Running and evaluating test cases": **eval workspaces live in `.claude/skill-workspaces/<skill-name>/`,
   not as a sibling of the skill directory.** Upstream's placement would create a directory inside
   `.claude/skills/` with no `SKILL.md`, which `audit.py` reports as an error. The path is git-ignored.
4. Body, "Reference files": added the pointer to `references/size-budget-table.md`.
5. `references/schemas.md` gained a table of contents — it is 431 lines, and upstream's own rule is that a
   reference over 300 lines carries one.

The body still writes script invocations as `python -m scripts.…` and `<skill-creator-path>/…`; resolve both
against `.claude/skills/skill-creator/` per section 3 above.
...
Put results in `.claude/skill-workspaces/<skill-name>/`. Within the workspace, organize results by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that, each test case gets a directory (`eval-0/`, `eval-1/`, etc.). Don't create all of this upfront — just create directories as you go.
...
- `references/size-budget-table.md` — this project's per-file-kind budgets and where each is enforced
